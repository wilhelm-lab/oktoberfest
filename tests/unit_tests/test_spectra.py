...
class TestSpectra:
    ...
    def test_assemble_for_parquet(self, mini_spectra, df_for_parquet):
        ...
        pd.testing.assert_frame_equal(df.reset_index(drop=True), df_for_parquet.reset_index(drop=True), check_column_type=False)
...
def df_for_parquet():
    ...
    df["intensities_raw"] = df["intensities_raw"].map(lambda intens: np.fromstring(intens.replace('\n','').replace('[','').replace(']','').replace('  ',' '), sep=' '))
    df["precursor_charge_onehot"] = df["precursor_charge_onehot"].map(lambda intens: np.fromstring(intens.replace('\n','').replace('[','').replace(']','').replace('  ',' '), dtype='int', sep=' '))
    return df