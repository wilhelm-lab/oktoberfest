<template>
  ...
  <v-stepper v-model="step" vertical>
    ...
    <Info>
      This task predicting a list of peptides given in a CSV file.
    ...
    <v-stepper-content step="4">
      <v-card flat>
        ...
        <v-card-actions>
          ...
          <Submit :taskid="taskid">Predict</Submit>
        </v-card-actions>
      </v-card>
    </v-stepper-content>
  ...
  methods: {
    ...
    setModels: function(response){
      ...
      for (let i = 0; i < modelIntensityLists.length; i++){
        //if(modelIntensityLists[i].enabled.predict_properties)
        this.modelIntensityList.push(modelIntensityLists[i]);
      }
      for (let i = 0; i < modelIRTLists.length; i++){
        //if(modelIRTLists[i].enabled.predict_properties)
